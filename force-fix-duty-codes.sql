...
-- 既存のユニーク制約を削除（存在する場合）
ALTER TABLE duty_codes DROP CONSTRAINT IF EXISTS duty_codes_code_key;

-- 複合ユニーク制約を追加（存在しない場合のみ）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'duty_codes_code_category_key'
    ) THEN
...
DROP INDEX IF EXISTS idx_duty_codes_code_category;
CREATE INDEX idx_duty_codes_code_category ON duty_codes(code, category);

-- =====================================================
-- ステップ2: データを完全にクリア（TRUNCATEで高速削除）
-- =====================================================

-- 外部キー制約がある場合はCASCADEで削除
TRUNCATE TABLE duty_codes CASCADE;

-- =====================================================
-- ステップ3: 修正したデータを投入（全31件）
-- =====================================================
...
BEGIN
  SELECT COUNT(*) INTO total_count FROM duty_codes;
  SELECT COUNT(*) INTO invalid_count FROM duty_codes WHERE LENGTH(code) != 6;

  RAISE NOTICE '=================================';
  RAISE NOTICE '勤務記号データ修正完了';
  RAISE NOTICE '=================================';
  RAISE NOTICE '総件数: % 件', total_count;
  RAISE NOTICE '無効なコード: % 件', invalid_count;

  IF invalid_count > 0 THEN
    RAISE WARNING '6文字でないコードが存在します！';
  ELSE
    RAISE NOTICE 'すべてのコードが6文字形式です ✓';
  END IF;
END $$;

-- 全データを表示して確認
SELECT code, category, start_time, end_time, is_overnight
FROM duty_codes
