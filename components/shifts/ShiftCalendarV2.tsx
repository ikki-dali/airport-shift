    ...
    <Card className="overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          <thead>
            <tr>
              <th className="sticky left-0 z-20 bg-gray-100 border-b-2 border-gray-300 p-3 text-sm font-semibold text-gray-700 w-32">
                配属箇所
              </th>
              {days.map((day) => (
                <th
                  key={day.toISOString()}
                  className={`border-b-2 border-gray-300 p-3 text-sm font-semibold w-40 ${getDayOfWeekClass(
                    day
                  )}`}
                >
          ...
          <tbody>
            {locations.map((location) => (
              <tr key={location.id} className="border-b border-gray-200">
                <td className="sticky left-0 z-10 bg-gray-50 border-r border-gray-300 p-3 font-medium text-sm">
